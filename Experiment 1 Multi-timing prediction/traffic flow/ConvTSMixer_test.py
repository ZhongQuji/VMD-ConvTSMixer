...
if __name__ == "__main__":
    ...
    savemat('results\\预测步长{}_ConvTSMixer133.mat'.format(length_size), {'true': true, 'pred': pred})
    result_finally = np.concatenate((true, pred), axis=1)
    print(result_finally.shape)
    df = pd.DataFrame(result_finally, columns=['real', 'pred'])
    df.to_csv('results\\预测步长{}_ConvTSMixer666.csv'.format(length_size), index=False)
    ...
    plt.legend()
    plt.savefig('images\\预测步长{}_ConvTSMixer666.png'.format(length_size), dpi=100)
    plt.show()
    ...
    savef['MAPE'] = [str(MAPE)]
    savef['R2'] = [str(R2)]
    savef.to_csv('error\\error_预测步长{}_ConvTSMixer666.csv'.format(length_size), index=False)