...
from dlinear.DLinear import Model
...
file_path = 'data\\FC\\Ocean.xlsx'
window = 12  # 模型输入序列长度
length_size = 12  # 预测结果的序列长度
epochs = 10  # 迭代次数
batch_size = 32
# 读取数据
df_raw = pd.read_excel(file_path)  # 读取目标文件
...
def data_loader(window, length_size, batch_size, data, data_mark):
    ...
    y_train = result[:, -length_size:]  # 训练集目标数据
    print('y_train shape:', y_train.shape)
    y_train_mark = result_mark[:, -length_size:]
    ...
    dataloader = torch.utils.data.DataLoader(ds, batch_size=batch_size,
                                             shuffle=True)  # 对训练集数据进行打包，每32个数据进行打包一次，组后一组不足32的自动打包
...
def dlinear_train(config):
    ...
    best_model_path = 'checkpoint/best_Dlinear.pt'
    torch.save(net.state_dict(), best_model_path)
    return net


def dlinear_test(config):
    net = Model(config).to(device)
    net.load_state_dict(torch.load('checkpoint/best_Dlinear.pt'))  # 加载训练好的模型
    net.eval()

    pred = net(x_test.to(device), None, None, None)
...
if __name__ == "__main__":
    dlinear_train(config)
    true, pred = dlinear_test(config)
    savemat('results\\预测步长{}_Dlinear.mat'.format(length_size), {'true': true, 'pred': pred})
    result_finally = np.concatenate((true, pred), axis=1)
    print(result_finally.shape)
    df = pd.DataFrame(result_finally, columns=['real', 'pred'])
    df.to_csv('results\\预测步长{}_dlinear.csv'.format(length_size), index=False)
    ...
    plt.legend()
    plt.savefig('images\\预测步长{}_idlinear.png'.format(length_size), dpi=100)
    plt.show()
    ...
    savef['MAPE'] = [str(MAPE)]
    savef['R2'] = [str(R2)]
    savef.to_csv('error\\error_预测步长{}_dlinear.csv'.format(length_size), index=False)
