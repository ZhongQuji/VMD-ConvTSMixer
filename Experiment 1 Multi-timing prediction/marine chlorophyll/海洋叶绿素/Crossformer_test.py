...
from Crossformer.Crossformer import Model
...
def Crossformer_train(config):
    ...
    best_model_path = 'checkpoint/best_Crossformer.pt'
    torch.save(net.state_dict(), best_model_path)
    return net
def Crossformer_test(config):
    net = Model(config).to(device)
    net.load_state_dict(torch.load('checkpoint/best_Crossformer.pt'))  # 加载训练好的模型
    net.eval()
...
if __name__ == "__main__":
    Crossformer_train(config)
    true, pred = Crossformer_test(config)
    savemat('results\\预测步长{}_Crossformer.mat'.format(length_size), {'true':true, 'pred': pred})
    result_finally = np.concatenate((true, pred), axis=1)
    print(result_finally.shape)
    df = pd.DataFrame(result_finally, columns=['real', 'pred'])
    df.to_csv('results\\预测步长{}_Crossformer.csv'.format(length_size), index=False)
    ...
    plt.title('Crossformer Quantile prediction results')
    plt.legend()
    plt.savefig('images\\预测步长{}_Crossformer.png'.format(length_size), dpi=100)
    plt.show()
    ...
    savef['MAPE'] = [str(MAPE)]
    savef['R2'] = [str(R2)]
    savef.to_csv('error\\error_预测步长{}_Crossformer.csv'.format(length_size), index=False)