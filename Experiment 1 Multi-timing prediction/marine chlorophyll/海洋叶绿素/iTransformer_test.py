...
from iTransformer.iTransformer import Model
...
def iTransformer_train(config):
    ...
    best_model_path = 'checkpoint/best_iTransformer.pt'
    torch.save(net.state_dict(), best_model_path)
    return net
def iTransformer_test(config):
    net = Model(config).to(device)
    net.load_state_dict(torch.load('checkpoint/best_iTransformer.pt'))  # 加载训练好的模型
    net.eval()
...
if __name__ == "__main__":
    iTransformer_train(config)
    true, pred = iTransformer_test(config)
    savemat('results\\预测步长{}_iTransformer.mat'.format(length_size), {'true':true, 'pred': pred})
    ...
    plt.title('iTransformer Quantile prediction results')
    plt.legend()
    plt.savefig('images\\预测步长{}_iTransformer.png'.format(length_size), dpi=100)
    plt.show()
    ...
    savef['MAPE'] = [str(MAPE)]
    savef['R2'] = [str(R2)]
    savef.to_csv('error\\error_预测步长{}_iTransformer.csv'.format(length_size), index=False)