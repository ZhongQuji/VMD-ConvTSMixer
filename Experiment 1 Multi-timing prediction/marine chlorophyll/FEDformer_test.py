...
from FEDformer.FEDformer import Model
...
def FEDformer_train(config):
    ...
    for epoch in range(epochs):  # 10
        for i, (datapoints, labels, datapoints_mark, labels_mark) in enumerate(dataloader_train):
            ...
            preds = preds[:,-length_size:,-1:].squeeze()
    ...
    best_model_path = 'checkpoint/best_FEDformer.pt'
    torch.save(net.state_dict(), best_model_path)
    return net


def FEDformer_test(config):
    net = Model(config).to(device)
    net.load_state_dict(torch.load('checkpoint/best_FEDformer.pt'))  # 加载训练好的模型
    net.eval()
...
if __name__ == "__main__":
    FEDformer_train(config)
    true, pred = FEDformer_test(config)
    savemat('results\\预测步长{}_FEDformer.mat'.format(length_size), {'true': true, 'pred': pred})
    result_finally = np.concatenate((true, pred), axis=1)
    print(result_finally.shape)
    df = pd.DataFrame(result_finally, columns=['real', 'pred'])
    df.to_csv('results\\预测步长{}_FEDformer.csv'.format(length_size), index=False)
    ...
    plt.title('FEDformer Quantile prediction results')
    plt.legend()
    plt.savefig('images\\预测步长{}_FEDformer.png'.format(length_size), dpi=100)
    plt.show()
    ...
    savef['MAPE'] = [str(MAPE)]
    savef['R2'] = [str(R2)]
    savef.to_csv('error\\error_预测步长{}_FEDformer.csv'.format(length_size), index=False)
