...
from ConvTSMixer.ConvTSMixer import Model
...
class Config:
    def __init__(self):
        ...
        self.e_layers = 5 #块的数量
        self.dec_in = n_feature
        self.enc_in = n_feature #特征维度
        self.c_out = 1
        self.d_model = 32 #
        self.dropout = 0.01
#        self.groups = 2  # 定义分组数量,分组卷积时候使用
config = Config()
...
if __name__ == "__main__":
    ...
    savemat('results\\预测步长{}_ConvTSMixer6.mat'.format(length_size), {'true': true, 'pred': pred})
    result_finally = np.concatenate((true, pred), axis=1)
    print(result_finally.shape)
    df = pd.DataFrame(result_finally, columns=['real', 'pred'])
    df.to_csv('results\\预测步长{}_ConvTSMixer6.csv'.format(length_size), index=False)
    ...
    plt.legend()
    plt.savefig('images\\预测步长{}_ConvTSMixer6.png'.format(length_size), dpi=100)
    plt.show()
    ...
    savef['MAPE'] = [str(MAPE)]
    savef['R2'] = [str(R2)]
    savef.to_csv('error\\error_预测步长{}_ConvTSMixer6.csv'.format(length_size), index=False)