...
def PatchTST_train(config):
    ...
    best_model_path = 'checkpoint/best_PatchTST.pt'
    torch.save(net.state_dict(), best_model_path)
    return net
def PatchTST_test(config):
    net = Model(config).to(device)
    net.load_state_dict(torch.load('checkpoint/best_PatchTST.pt'))  # 加载训练好的模型
    net.eval()
...
if __name__ == "__main__":
    PatchTST_train(config)
    true, pred = PatchTST_test(config)
    savemat('results\\预测步长{}_PatchTST.mat'.format(length_size), {'true': true, 'pred': pred})
    result_finally = np.concatenate((true, pred), axis=1)
    print(result_finally.shape)
    df = pd.DataFrame(result_finally, columns=['real', 'pred'])
    df.to_csv('results\\预测步长{}_PatchTST.csv'.format(length_size), index=False)
    ...
    plt.title('PatchTST Quantile prediction results')
    plt.legend()
    plt.savefig('images\\预测步长{}_PatchTST.png'.format(length_size), dpi=100)
    plt.show()
    ...
    savef['MAPE'] = [str(MAPE)]
    savef['R2'] = [str(R2)]
    savef.to_csv('error\\error_预测步长{}_PatchTST.csv'.format(length_size), index=False)